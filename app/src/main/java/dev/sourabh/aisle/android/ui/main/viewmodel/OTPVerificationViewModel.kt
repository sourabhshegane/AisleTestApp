...
class OTPVerificationViewModel(private val mainRepository: MainRepository) : ViewModel() {
    ...
    fun requestOTPValidation(otpVerificationRequest: OTPValidationRequest) = liveData(Dispatchers.IO) {
        emit(Resource.loading(data = null))
        try {
            emit(Resource.success(data = mainRepository.validateOTP(otpVerificationRequest)))
        }catch (exception: Exception){
            emit(Resource.error(data = null, message = exception.message ?: "An error occurred"))
        }
    }
